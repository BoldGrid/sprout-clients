...
if ( class_exists( 'SC_Records_Table' ) ) {
	return;
}
class SC_Records_Table extends WP_List_Table {
	protected static $post_type = SC_Record::POST_TYPE;
	...
	function column_data( $item ) {
		...
		if ( $data != '' ) {
			?>
				<a href="#TB_inline?width=900&height=600&inlineId=data_id_<?php echo esc_attr( $item->ID ); ?>" class="thickbox button" title="<?php echo esc_attr( $item->post_title ); ?> <?php sc_e( 'Data' ) ?>"><?php sc_e( 'View Data' ) ?></a>

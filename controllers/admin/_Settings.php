 ...
 * @subpackage Settings
 */
class SC_Settings_API extends SC_Controller {
	...
	public static function add_admin_page() {
		...
		self::$settings_page = add_submenu_page( 'options-general.php', __( 'Sprout Clients', 'sprout-invoices' ), __( 'Sprout Clients', 'sprout-invoices' ), 'manage_sprout_clients_options', self::APP_DOMAIN, array( __CLASS__, 'default_admin_page' ) );

		if ( class_exists( 'Sprout_Invoices' ) ) {
			add_filter( 'si_sub_admin_pages', array( __CLASS__, 'register_siadmin_page' ) );
		}

	}
	public static function register_siadmin_page( $admin_pages = array() ) {
		$admin_pages['sa_clients'] = array(
			'slug' => 'sa_clients',
			'title' => __( 'Sprout Clients', 'sprout-invoices' ),
			'menu_title' => __( 'Sprout Clients', 'sprout-invoices' ),
			'weight' => 35,
			'reset' => false,
			'section' => 'settings',
			'tab_only' => true,
			'callback' => array( __CLASS__, 'redirect_for_si_admin' ),
			);
		return $admin_pages;
	}

	public static function redirect_for_si_admin() {
		printf( '<script type="text/javascript">window.location = "%s";</script>', admin_url( 'options-general.php?page=sprout-clients' ) );
		_e( 'You\'re being redirected to the Sprout Clients admin. Settings > Sprout Clients.', 'sprout-clients' );
	}
